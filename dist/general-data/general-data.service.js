"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GeneralDataService = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
const user_schema_1 = require("../user/user.schema");
const general_data_schema_1 = require("./schema/general-data.schema");
let GeneralDataService = class GeneralDataService {
    constructor(generalDataModel, userModel) {
        this.generalDataModel = generalDataModel;
        this.userModel = userModel;
    }
    async createGeneralData(createGeneralDataDto, userId) {
        const user = await this.userModel.findById(userId);
        if (!user) {
            throw new common_1.NotFoundException('User not found');
        }
        const generalData = new this.generalDataModel({
            ...createGeneralDataDto,
            user: user._id,
        });
        return generalData.save();
    }
    async getGeneralDataByUserId(userId) {
        return this.generalDataModel.findOne({ user: userId }).populate('user');
    }
    async updateGeneralData(userId, updateDto) {
        const data = await this.generalDataModel
            .findOneAndUpdate({ user: userId }, { ...updateDto }, { new: true })
            .populate('user');
        if (!data) {
            this.createGeneralData(updateDto, userId);
        }
        return data;
    }
};
exports.GeneralDataService = GeneralDataService;
exports.GeneralDataService = GeneralDataService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)(general_data_schema_1.GeneralData.name)),
    __param(1, (0, mongoose_1.InjectModel)(user_schema_1.User.name)),
    __metadata("design:paramtypes", [mongoose_2.Model,
        mongoose_2.Model])
], GeneralDataService);
//# sourceMappingURL=general-data.service.js.map